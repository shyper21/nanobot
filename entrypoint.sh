...
# Generate config dari Railway environment variables
mkdir -p "$dir"
python3 - <<'PYEOF'
import os, json
cfg = {
    ...
    "channels": {"telegram": {"enabled": True, "token": os.environ.get("TELEGRAM_BOT_TOKEN", ""), "allowFrom": [os.environ.get("TELEGRAM_USER_ID", "")]}}
}
